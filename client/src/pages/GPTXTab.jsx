...
import {
  Send, Sparkles, ImageIcon, Video, RefreshCw, Trash2, Plus,
  ChevronDown, X, Copy, Download, Check, Loader2, AlertCircle,
  MessageSquare, Wand2, Camera, Film, ChevronLeft, Menu,
} from "lucide-react";
    ...
    >
      ...
      {hovering && (
        <button
          ...
          className="p-1 rounded-md hover:bg-white/10 text-slate-500 hover:text-rose-400 transition-colors shrink-0 mt-0.5"
        >
...
function AssistantMessage({ msg, onMakeVideo, onUseAsRef, models }) {
  ...
  if (!gen) {
    return (
      <div className="flex items-start gap-2.5 max-w-[85%]">
        <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5" style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)" }}>
        ...
        <div className="px-4 py-3 rounded-2xl rounded-tl-sm" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center gap-2 text-slate-400">
            ...
            <span className="text-[12.5px]">Generating your image…</span>
          </div>
        </div>
      </div>
    );
  ...
  if (gen.status === "failed") {
    return (
      <div className="flex items-start gap-2.5 max-w-[85%]">
        ...
        <div className="px-4 py-3 rounded-2xl rounded-tl-sm" style={{ background: "rgba(239,68,68,0.06)", border: "1px solid rgba(239,68,68,0.15)" }}>
          <div className="flex items-center gap-2 text-rose-400">
            <AlertCircle className="w-4 h-4" />
            <span className="text-[12.5px]">Generation failed. {gen.errorMessage || "Please try again."}</span>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="flex items-start gap-2.5 max-w-[85%]">
        ...
        <div className="px-4 py-3 rounded-2xl rounded-tl-sm" style={{ background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.06)" }}>
          <div className="flex items-center gap-2 text-slate-400">
            <Loader2 className="w-4 h-4 animate-spin text-violet-400" />
            <span className="text-[12.5px]">Generating image… this takes ~30s</span>
          </div>
        </div>
      </div>
    );
        ...
        {msg.videoGenId && (
          <div className="mt-2">
            {!videoGen || videoGen.status !== "completed" ? (
              <div className="flex items-center gap-2 px-3 py-2 rounded-xl text-[11.5px] text-slate-400" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
...
export default function GPTXTab() {
  ...
  const loadConversation = async (convId) => {
    try {
      ...
      // Restore any pending generations from message data
      // (only relevant if page reloads mid-generation, handled via polling)
    } catch (err) {
  ...
  const doSend = async (convId) => {
    ...
    try {
      ...
      try {
        // Map aspectRatio to resolution for NSFW
  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] md:h-[calc(100vh-3.5rem)] overflow-hidden rounded-2xl" style={{ background: "var(--bg-content)", border: "1px solid var(--border-subtle)" }}>
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto px-2 pb-3 space-y-0.5 scrollbar-thin">
              {conversations.length === 0 ? (
                <p className="text-[11px] text-slate-600 text-center py-6 px-2">No chats yet. Start below!</p>
              ) : (
                conversations.map(c => (
                  <ConvItem
                    key={c.id}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="flex items-center gap-3 px-4 py-2.5 border-b shrink-0" style={{ borderColor: "var(--border-subtle)" }}>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-semibold text-white">GPT-X Studio</span>
            <span className="px-1.5 py-0.5 rounded-md text-[9px] font-bold tracking-wider uppercase" style={{ background: "rgba(124,58,237,0.15)", color: "#a78bfa", border: "1px solid rgba(124,58,237,0.3)" }}>Beta</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5 scrollbar-thin">
          {!activeConvId && messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4 pb-8">
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: "linear-gradient(135deg, rgba(124,58,237,0.2), rgba(79,70,229,0.1))", border: "1px solid rgba(124,58,237,0.2)" }}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 w-full max-w-sm">
                {[
                  "Mirror selfie in a modern apartment at night",
                  ...
                  <button
                    ...
                    className="text-left px-3 py-2 rounded-xl text-[11.5px] text-slate-400 hover:text-white transition-colors"
            ...
            if (msg.role === "user") {
              return (
                <div key={msg.id} className="flex justify-end">
                  <div className="max-w-[75%] px-4 py-2.5 rounded-2xl rounded-tr-sm text-[13px] text-white" style={{ background: "rgba(124,58,237,0.2)", border: "1px solid rgba(124,58,237,0.25)" }}>
                    {msg.content}
                  </div>
                </div>
              );
        ...
        <div className="shrink-0 px-3 pb-3 pt-2 border-t" style={{ borderColor: "var(--border-subtle)" }}>
          ...
          {referenceImageUrl && (
            <div className="flex items-center gap-2 mb-2 px-2 py-1.5 rounded-xl" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.06)" }}>
              <Camera className="w-3.5 h-3.5 text-violet-400 shrink-0" />
              <img src={referenceImageUrl} alt="ref" className="w-8 h-8 rounded-lg object-cover" />
              <span className="text-[11px] text-slate-400 flex-1 truncate">Using as reference</span>
              <button onClick={() => setReferenceImageUrl(null)} className="p-1 rounded-md hover:bg-white/10 text-slate-500 hover:text-white transition-colors">
                <X className="w-3 h-3" />
              </button>
            </div>
          )}
          ...
          <div className="flex items-end gap-2">
            <div className="flex-1 relative">
              <textarea
                ...
                placeholder={hasModels ? "Describe what you want to create…" : "Select a model above to get started"}
                disabled={sending || !hasModels}
                rows={1}
            ...
            <button
              onClick={handleSend}
              disabled={!inputText.trim() || sending || !hasModels}
              className="h-[42px] w-[42px] rounded-xl flex items-center justify-center transition-all disabled:opacity-30 disabled:cursor-not-allowed shrink-0"
              style={{ background: "linear-gradient(135deg, #7c3aed, #4f46e5)" }}
            >
              {sending ? <Loader2 className="w-4 h-4 text-white animate-spin" /> : <Send className="w-4 h-4 text-white" />}
            </button>
          </div>
          <p className="text-[10px] text-slate-600 mt-1.5 text-center">Grok enhances your prompt • results appear here when ready • Shift+Enter for new line</p>
        </div>
      </div>
    </div>
  );
